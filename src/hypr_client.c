...
static inline bool
get_hypr_instance_path(char* result, const int result_len) {
...
static inline bool
on_parse_start(HyprParser* parser) {
  ASSERT(parser);
  fprintf(stdout, "parsing hyprland event:\n%s\n", (const char*)parser->data);
  return true;
}

static inline bool
on_parse_finished(HyprParser* parser) {
  ASSERT(parser);
  fprintf(stdout, "parsing finshed\n");
  return true;
}

static inline bool
on_event(HyprParser* parser, const char* event, const uint64_t event_len) {
  ASSERT(parser);
  fprintf(stdout, "parsed event: %s\n", event);
  return true;
}

static inline bool
on_data(HyprParser* parser, const char* data, const uint64_t data_len) {
  ASSERT(parser);
  fprintf(stdout, "parsed event data: %s\n", data);
  return true;
}
...
on_read(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf) {
  if(nread > 0) {
    HyprParser parser;
    hypr_parser_init(&parser, (const uint8_t*)buf->base, buf->len, 
                     on_parse_start,
                     on_event, // on_event
                     NULL,  // on_data_start
                     on_data,  // on_data0
...
static inline void
connect_to_hypr_socket(HyprClient* client, uv_pipe_t* pipe, const char* socket) {
  ASSERT(client);
  ASSERT(socket);
